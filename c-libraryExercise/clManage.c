...
int main(int argc, char *argv[]) {
  static struct option long_options[] = {   //Definiamo i parametri ammessi
  ...
  static char *short_options = "vha:s:";    //Definizione delle opzioni "corte"
  ...
  //optarg punta la "value" dopo [OPTION]
  while((opt = getopt_long(argc, argv, short_options, long_options, NULL)) > 0) {
    //Opzioni da eseguire in base alle varie option 
    switch (opt) {
      case 'v': verbose = 1;
                break;
  ...
  //getopt() pone tutti gli argomenti alla fine dell'array
  //e tutte le options all'inizio
